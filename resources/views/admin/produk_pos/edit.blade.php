...
<div class="px-4 sm:px-6 lg:px-8 max-w-4xl">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold text-gray-900 mt-2">Edit Produk POS: <span class="text-green-700">{{ $produk_pos->nama_produk }}</span></h1>
    </div>
    ...
    <form action="{{ route('admin.produk-pos.update', $produk_pos->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-gray-100">
            ...
            <div class="col-span-2 p-6 space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="sku" name="sku" value="{{ old('sku', $produk_pos->sku) }}"
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_produk" name="nama_produk" value="{{ old('nama_produk', $produk_pos->nama_produk) }}"
                    ...
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            ...
                            <input type="number" id="harga" name="harga" value="{{ old('harga', $produk_pos->harga) }}"
                                   min="0"
                    ...
                    <div>
                        ...
                        <select id="kategori" name="kategori"
                            ...
                            @foreach(['Dine-in','Minuman','Snack'] as $kat)
                                <option value="{{ $kat }}" @selected(old('kategori', $produk_pos->kategori) === $kat)>{{ $kat }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="text" id="badge" name="badge" value="{{ old('badge', $produk_pos->badge) }}"
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                              class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('deskripsi', $produk_pos->deskripsi) }}</textarea>
                </div>
            </div>
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <div class="mb-4">
                        <input type="url" id="gambar_url" name="gambar_url" value="{{ old('gambar_url', str_starts_with($produk_pos->gambar ?? '', 'http') ? $produk_pos->gambar : '') }}"
                    ...
                    <div id="preview-container" class="mb-3 {{ $produk_pos->gambar ? '' : 'hidden' }}">
                        <p class="text-xs text-gray-500 mb-1">Gambar saat ini / Preview:</p>
                        <img id="preview-img" src="{{ $produk_pos->gambar ? (str_starts_with($produk_pos->gambar, 'http') ? $produk_pos->gambar : Storage::url($produk_pos->gambar)) : '' }}" 
        ...
        <div class="px-6 py-4 bg-gray-50 rounded-b-lg flex items-center justify-between border-t border-gray-100">
            <div>
                ...
                <button type="button" onclick="confirmDelete()" class="text-sm text-red-600 hover:text-red-800 font-medium">Hapus Produk POS ini</button>
            </div>
...
<form id="delete-form" action="{{ route('admin.produk-pos.destroy', $produk_pos->id) }}" method="POST" class="hidden">
    @csrf
    @method('DELETE')
</form>
...
function confirmDelete() {
    if(confirm('Yakin ingin menghapus Produk POS ini?')) {
